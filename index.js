...
teks = args.join(' ')
sticWait(from)
sendMediaURL(from, `https://api-rull.herokuapp.com/api/ssweb?url=${teks}`)
            break
...
case 'nsfw_loli':   
ini_buffer = await getBuffer(`https://api.reysekha.xyz/api/wallpaper/nsfwloli?apikey=apirey`)
...
case 'nsfw_neko':   
ini_buffer = await getBuffer(`https://waifu.pics/api/nsfw/neko`)
...
buf = await fetchJson(`https://api.reysekha.xyz/api/downloader/wallpaperflare?query=${search}&apikey=apirey`)

ft = buf.result

diaa = ft[Math.floor(Math.random() * ft.length)]
			
sendMediaURL(from, `${diaa}`)
break
...
try {
	...
	if (args.length === 0) return reply(`*Ejemplo De Uso:* ${prefix + command} <link tiktok>`)
